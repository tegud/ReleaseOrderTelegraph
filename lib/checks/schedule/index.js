...
function ScheduleChecker(config, eventEmitter) {
    function checkCurrentScheduleState() {
        const currentTime = moment();

        const matchingSchedule = matchSchedule(config, currentTime);
        const matchingOverride = matchOverride(config, currentTime);

        buildCheckResponse(config, matchingSchedule, matchingOverride)
            .then(setNextChange.bind(undefined, config, currentTime))
            .then(result => {
                if(result.nextChange) {
                    setTimeout(checkCurrentScheduleState, moment(result.nextChange.changeAt).diff(currentTime, 'ms'));
                }
    ...
    return {
        start: () => checkCurrentScheduleState()
    };
}
...
module.exports = function(config, eventEmitter) {
    ...
    const checker = new ScheduleChecker(config, eventEmitter);
