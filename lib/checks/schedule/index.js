...
function setNextChange(config, result) {
    ...
    if(daySchedule) {
        ...
        if(matchingSchedule) {
            ...
            console.log(matchingSchedule.to.format());
            ...
            const firstSchedule = getFirstScheduleForDay(config, today, result.signal, currentTime);

            if(firstSchedule) {
                result.nextChange = {
                    changeAt: firstSchedule.from.format(),
                    toSignal: firstSchedule.signal || 'green'
                };
            }
            else {
                const firstSchedule = getFirstScheduleForDay(config, moment(today).add(1, 'd'), result.signal);
    ...
    return {
        getState: () => {
            ...
            return buildCheckResponse(config, matchingSchedule, matchingOverride)
                .then(setNextChange.bind(undefined, config));
        }
    };
};
